...
namespace llvm {
void initializeLoopFullDSEPass(llvm::PassRegistry &);
}
...
class LoopFullDSE : public ModulePass {
  // Region *region;
  // Scop *S;
  DominatorTree *DT;
  ScalarEvolution *SE;
...
/*
*/
char LoopFullDSE::ID = 1;

INITIALIZE_PASS_BEGIN(LoopFullDSE, "loop-full-dse", "Leandro's single loop full DSE",false, false)

// INITIALIZE_PASS_DEPENDENCY(SchedulerDAG)
INITIALIZE_PASS_END(LoopFullDSE, "loop-full-dse", "Leandro's single loop full DSE",false, false)

static RegisterPass<LoopFullDSE> Z("loop-full-dse", "Leandro's single loop full DSE");
