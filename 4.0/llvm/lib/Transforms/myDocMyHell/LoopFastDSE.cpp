...
void LoopFastDSE::clearLoopData(LoopData *ld){
...
void LoopFastDSE::printLoopsData(){
  for(auto entry : loopDataMap){
...
void LoopFastDSE::printLoopData(LoopData *ldin){
  std::cout << "\nloop: " << ldin->label << "\n";
  ldin->loop->dump();
...
LoopData * LoopFastDSE::getLoopBasicMetrics(llvm::Loop * loop){
...
void LoopFastDSE::addResourcesConstraints(LoopData *ld){
  ...
  for(auto entry : ld->nFUs){
    ...
    //this will generate just one constraint file with the maximum resources
    constraint = new Constraint(constrName, ub, ub);
    ld->TCLConstraints.push_back(constraint);
  }
...
void LoopFastDSE::addPipelineConstraint(LoopData *ld){
...
std::vector<Constraint*> LoopFastDSE::parseConstraints(LoopData *ld){
...
void LoopFastDSE::createTCLConfigs(llvm::Loop *loop){
  ...
  std::string basename = std::string("fast_loops_out/");
...
void LoopFastDSE::createMakefile(llvm::Loop *loop){
  ofstream file;
  std::string basename = std::string("fast_loops_out/");
  basename.append(loopDataMap[loop]->label);
  ...
  file << "include $(LEGUPHOME)/examples/Makefile.loopsFast\n";
  file.close();
  return;
}

void LoopFastDSE::saveLoopAsFunction(llvm::Loop *loop){

  std::string outdir = std::string("fast_loops_out");
...
bool LoopFastDSE::runOnModule(Module &M){
    module = &M;
