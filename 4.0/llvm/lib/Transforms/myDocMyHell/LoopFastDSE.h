...
namespace llvm {
void initializeLoopFastDSEPass(llvm::PassRegistry &);
}
...
class LoopFastDSE : public ModulePass {
  // Region *region;
  // Scop *S;
  DominatorTree *DT;
  ScalarEvolution *SE;
...
/*
*/
char LoopFastDSE::ID = 1;

INITIALIZE_PASS_BEGIN(LoopFastDSE, "loop-fast-dse", "Leandro's single loop fast DSE",false, false)

// INITIALIZE_PASS_DEPENDENCY(SchedulerDAG)
INITIALIZE_PASS_END(LoopFastDSE, "loop-fast-dse", "Leandro's single loop fast DSE",false, false)

static RegisterPass<LoopFastDSE> Z("loop-fast-dse", "Leandro's single loop fast DSE");
